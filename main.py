...
def trace_ray(ray_origin, ray_dir, spheres,x,y,depth=1,max_bounces=10):
    if depth > max_bounces:
        return np.array([0, 0, 0])  # Black color if maximum depth is exceeded

    background_color = np.array([135, 206, 235])  # Sky blue
    background_color = np.array([0, 0, 0, 0])
    color = background_color  # Default to background color if no intersection
    nearest_t = float('inf')
    epsilon = 1e-4  # Small bias to prevent self-intersections
    for sphere in spheres:
        ...
        if t and t < nearest_t:
            ...
            # Lighting calculation only if sun is present
            # Two-sided lighting: flip normal if facing away
            if np.dot(ray_dir, normal) > 0:
                normal = -normal

            if gpusettings.light_direction is not None:
                shadow_ray_origin = P + normal * epsilon  # Bias for shadow ray
                in_shadow = False

                # Check if the shadow ray hits any other object
                for other_sphere in spheres:
                    if other_sphere != sphere:
                        shadow_t = other_sphere.intersect(shadow_ray_origin, gpusettings.light_direction,x,y)
                        # Ignore intersections where t < epsilon
                ...
                if in_shadow :
                    ...
                    color = sphere.color  # Dim color if in shadow *0.1
                #print("light dir: ",gpusettings.light_direction)

                else:
                    # Calculate reflection first if shininess is present
                    reflection_color = [1,1,1,1]  # Default reflection color if none is calculated
                    if (x == pixelTestx and y == pixelTesty):
                        print("sphere shin: ", sphere.shininess)
                    if sphere.shininess and max(sphere.shininess) > 0:
                        reflection_dir = ray_dir - 2 * np.dot(ray_dir, normal) * normal
                        reflection_dir = reflection_dir / np.linalg.norm(reflection_dir)
                        if(x==pixelTestx and y==pixelTesty):
                            print("reflection_dir",reflection_dir)
                        reflection_color = trace_ray(P + normal * epsilon, reflection_dir, spheres, x, y, depth + 1, max_bounces)

                        # Ensure reflection_color is RGB by clipping if needed
                        reflection_color = reflection_color[:3] if len(reflection_color) > 3 else reflection_color
                    if (x==pixelTestx and y==pixelTesty):
                        print("rf color: ", reflection_color)
                    # Calculate diffuse color in linear space
                    diffuse_intensity = max(np.dot(normal, gpusettings.light_direction), 0)
                    #light_color = np.array(gpusettings.light_color)  # Ensure light_color is RGB and normalized
                    #diffuse_color = np.array(sphere.color) * diffuse_intensity * light_color
                    linear_color = sphere.color * diffuse_intensity * gpusettings.light_color  # Apply sun color in linear space

                    # Combine diffuse and reflection colors based on shininess in linear space
                    if sphere.shininess and max(sphere.shininess) > 0:
                        reflection_factor = np.array(sphere.shininess)
                        if (x==pixelTestx and y==pixelTesty):
                            print("stuff:",reflection_factor,linear_color,reflection_color)
                        linear_color = (1 - reflection_factor) * linear_color + reflection_factor * reflection_color

                    # Convert final color to sRGB and scale to 0–255
                    color = linear_to_srgb(linear_color) * 255
                    color = np.append(color[:3], 255)  # Ensure RGBA format with full opacity
                    #color=linear_color

            else:
                color = sphere.color  # Use unlit color if no sun
    if (x == pixelTestx and y == pixelTesty):
        print("color of stuff here: ", color)
...
def render(spheres, image_size, output_filename):
    ...
    for y in range(height):
        for x in range(width):
            ...
            # Perform sRGB conversion here

            color = trace_ray(ray_origin, ray_direction, spheres,x,y)
            #color = linear_to_srgb(color) * 255
...
if __name__ == "__main__":
    spheres, image_size, output_filename = read_scene("ray-shine1.txt")
