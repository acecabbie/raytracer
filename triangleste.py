...
def trace_ray(ray_origin, ray_dir, spheres, planes,triangles, x, y, depth=1, max_bounces=10):
    ...
    for triangle in triangles:
        t = triangle.intersect(ray_origin, ray_dir)
        if t and t < nearest_t:
            nearest_t = t
            hit_object = triangle
            P = ray_origin + t * ray_dir
    ...
    for light_direction, light_color in zip(gpusettings.light_directions, gpusettings.light_colors):
        ...
        for triangle in triangles:
            if triangle != hit_object:
                shadow_t = triangle.intersect(shadow_ray_origin, light_direction)
                if shadow_t and shadow_t >= epsilon:
                    in_shadow = True
                    break
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if parts[0] == "png":
                ...
                triangles.append(Triangle(v1, v2, v3, color))
...
if __name__ == "__main__":
    spheres,planes,triangles, image_size, output_filename = read_scene("ray-tex.txt")
